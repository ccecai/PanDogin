...
int ExecuteJump(uint8_t JumpType,float JumpAngle)
{
    if (JumpType == Bridge_Jump)//标准跳（对绝大多数地面都有较好的适应性，兼具高度和远度）
    {
        ...
        AngleLoop[5].Output_limit = 28.4f;
        AngleLoop[6].Output_limit = 28.4f;
        AngleLoop[7].Output_limit = 28.3f;
        AngleLoop[8].Output_limit = 28.3f;
    ...
    else if(JumpType == StepUp_Jump)
    {
        ...
        //芜湖起飞（核心），持续时间为launch_time
        AllLegsSpeedLimit(29.5f);//速度拉
        AngleLoop[5].Output_limit = 28.0f;
        AngleLoop[6].Output_limit = 28.0f;
        AngleLoop[7].Output_limit = 27.9f;
        AngleLoop[8].Output_limit = 27.9f;
    ...
    else if(JumpType == StepDown_Jump)//简单跳个远（要求地面摩擦较大）
    {
        ...
        const float stance_height = LegLenthMin;  //跳跃之前腿的高度  [cm]，理论上应等于LegSquatLenth 11.2f，这里测试跳跃时可以使用LegLenthMin 10.7f
        const float jump_extension = LegLenthExtremeMax; //伸展跳跃的最大伸腿长度      [cm]，理论上应等于LegLenthMax 28
        const float jump_flylegheight =
                LegStandLenth + 2.0f; //飞翔时腿长度   [cm]，经验上应介于LegLenthMax 28与LegStandLenth 18.0f之间，是一个适中的值。
        const float jump_landlegheight = LegStandLenth; //落地时腿长度  [cm]，理论上应等于LegStandLenth
        //下蹲，准备起跳，持续时间为prep_time
        AllLegsSpeedLimit(SpeedMode_EARLYEX);
        ChangeGainOfPID(7.3f, 1.0f, 0, 0);//使用刚度小，阻尼大的增益
        ...
        //芜湖起飞（核心），持续时间为launch_time
        AllLegsSpeedLimit(28.5f);//速度拉
        AngleLoop[5].Output_limit = 27.0f;
        AngleLoop[6].Output_limit = 27.0f;
        AngleLoop[7].Output_limit = 26.9f;
        AngleLoop[8].Output_limit = 26.9f;
...
int FrontJump(void )
{
    ...
    SetPolarPositionLeg_Delay(65.0f, 13.2f, prep_time,0);
    SetPolarPositionLeg_Delay(65.0f, 13.2f, prep_time,2);
    ...
    wait_flag = 1;
    AngleLoop[1].Output_limit = 23.75f;
    AngleLoop[2].Output_limit = 23.75f;
    AngleLoop[5].Output_limit = 23.75f;
    AngleLoop[6].Output_limit = 23.75f;
    SetPolarPositionLeg_Delay(65.0f, LegLenthExtremeMax, launch_time,0);
    SetPolarPositionLeg_Delay(65.0f, LegLenthExtremeMax, launch_time,2);
 ...
 * 缩腿过高栏
 */
    SetPolarPositionLeg_Delay(-48.0f, 7.5f, 0,1);
    SetPolarPositionLeg_Delay(-48.0f, 7.5f, 0,3);
